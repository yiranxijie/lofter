		...
		return this.each(function(){
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
		            ...
		            html += '					<div class="upload_btn" onclick="showpic()">开始上传</div>';
		            html += '				</div>';
		            html += '			</div>';
			...
			this.createCorePlug = function(){
				var params = {
					...
					onSuccess: function(file, request) {
						$("#uploadProgress_" + file.index).hide();
						$("#uploadSuccess_" + file.index).show();
						$("#pathpic").val(request.responseText);
						//
						
						$("#uploadInf").append("<p>上传成功，文件地址是：" + request.responseText + "</p>");
